...
export const signup = async (req, res) => {
  try {
    ...
    await data.save();
    return res.status(status.CREATED).json({ success: true, message: "Account Created Successfully" });
  } catch (error) {
    console.log(error);
...
export const login =  async (req, res) => {
    try {
        ...
        if(!email || !password){
            return res.status(400).json({success : false, message : "fill the details"});
        }
